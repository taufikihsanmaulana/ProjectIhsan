...
if ($_GET['aksi'] == 'tambah') {
    ...
    if ($_FILES['gambar']["error"] > 0) {
        ...
        if (move_uploaded_file($tmp_name, $dir.$name)) {
            $data[] = $_POST['no_plat'];
            $data[] = $_POST['nama_mobil'];
            $data[] = $_POST['merk'];
            $data[] = $_POST['deskripsi'];
            $data[] = $_POST['status_mobil'];
            $data[] = $name;

            $sql = "INSERT INTO `mobil`(`no_plat`, `nama_mobil`, `merk`, `deskripsi`, `status_mobil`, `gambar`) 
                VALUES (?,?,?,?,?,?)";
...
if ($_GET['aksi'] == 'edit') {
    ...
    $data[] = $id;
    $sql = "UPDATE mobil SET no_plat= ?, nama_mobil=?, merk=?, deskripsi=?, status_mobil=?, gambar=?
        WHERE id_mobil = ?";
    $row = $koneksi->prepare($sql);
    $row->execute($data);
